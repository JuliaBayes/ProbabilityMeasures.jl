...
exponential, uniform, Laplace, Cauchy, and categorical measures, and the multivariate
normal.
...
`Normal(μ, σ)`, `Exponential(θ)`, `Uniform(a, b)`, `Laplace(μ, b)`, `Cauchy(μ, σ)`, and
`Categorical(p)` each support:
...
`Laplace`, `Cauchy`, `Categorical`, and `MvNormal`. Transformed or composite measures and
